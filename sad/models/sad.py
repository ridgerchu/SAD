...
from sad.models.temporal_model import TemporalModelIdentity
from sad.models.distributions_snn import DistributionModule
from sad.models.future_prediction_snn import FuturePrediction
from sad.models.decoder_snn_ms import Decoder
from sad.models.planning_model import Planning
from sad.utils.network import pack_sequence_dim, unpack_sequence_dim, set_bn_momentum
from sad.utils.geometry import calculate_birds_eye_view_parameters, VoxelsSumming, pose_vec2mat
from sad.models.encoder_snn_merge import *
...
class SAD(nn.Module):
    def __init__(self, cfg):
        ...
        checkpoint = torch.load(self.cfg.MODEL.ENCODER.NAME, map_location="cpu")
...
#         pdb.set_trace()
        ...
        #  Temporal model
        b, s, c, h, w = x.shape 
        states = self.temporal_model(x) #[6, 3, 70, 200, 200]
...
#         pdb.set_trace()
        ...
        x = x.view(s * b * n, c, h, w) # (9 * 6, 3, 224, 480)
        x = x.unsqueeze(0)
        ...
        if self.cfg.MODEL.ENCODER.USE_DEPTH_DISTRIBUTION:
            ...
            x = depth_prob.unsqueeze(1) * x.unsqueeze(2)  # outer product depth and featencoder_forwardures
